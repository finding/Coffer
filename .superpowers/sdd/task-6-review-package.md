...
## Commit History
```
11d8d66 feat: add injected script for fetch/XHR interception
```

## Diff Stats
```
 src/content/bodyRewriter.ts   |   6 +-
 src/content/injectedScript.ts | 442 ++++++++++++++++++++++++++++++++++++++++++
 2 files changed, 446 insertions(+), 2 deletions(-)
```

## Full Diff
diff --git a/src/content/bodyRewriter.ts b/src/content/bodyRewriter.ts
index 70530b8..3715ae5 100644
--- a/src/content/bodyRewriter.ts
+++ b/src/content/bodyRewriter.ts
@@ -50,21 +50,22 @@ function setByPath(obj: Record<string, unknown>, path: string, value: string): v
...
@@ -120,21 +121,22 @@ export function rewriteGetParams(
...
+            currentQueryString = currentQueryString.split(rewrite.find).join(rewrite.replace || '')
         ...
         case 'regex': {
           ...
           break
diff --git a/src/content/injectedScript.ts b/src/content/injectedScript.ts
new file mode 100644
index 0000000..8716feb
--- /dev/null
+++ b/src/content/injectedScript.ts
@@ -0,0 +1,442 @@
